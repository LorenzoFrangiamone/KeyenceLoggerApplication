...
There is no test runner / CI configured. `tests/testAI.py` is a manual smoke-test script (not pytest) that exercises `AICorrection` directly — run it with `python tests/testAI.py`. It calls `AICorrector.load_model(...)` explicitly (model loading is no longer eager, see below) before calling `AICorrection`, and requires the GGUF model file referenced there to be present under `models/`, so it will fail fast without it.

## Building the portable executable

`build_portable.bat` runs PyInstaller (using `KCompareAgent.spec`-equivalent flags: `--onedir --windowed --collect-all llama_cpp`) and then copies every `.gguf` file in `models/` (`copy models\*.gguf ...`) into `dist/KCompareAgent/models/`, so all models available at build time are bundled and selectable via the in-app dropdown (see `src/modelSelector.py` below) — not just one hardcoded model.
